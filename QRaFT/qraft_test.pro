...
PRO QRaFT_TEST, key, fname, fname_B1, fname_B2, rho_min
  ...
  IMG_lbl_arr = detect_blobs(IMG_d2_phi_enh, p_arr, rho_min_arr, 5, blob_stat=blob_stat, blob_indices=blob_indices)
    ...
    if key le 1  then begin ; PSI model only
      ...
      save, filename = fname_save, features, angle_err, angle_err_signed, IMG_d2_phi_r, blob_stat, blob_indices, XYCenter, d_phi, rot_angle, phi_shift, smooth_xy, smooth_phi_rho, detr_phi, rho_range, r_rho, p_range, n_p
      ;-------------------------------------
